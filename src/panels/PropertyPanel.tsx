...
function CableCard({
   ...
   * 도면에 자켓이 그려지는가.
   *
   * 자켓은 심선이 **나란히 가는 구간**에만 그려진다(canvas/wirePlan). 레인 배정이
   * 같은 케이블 심선을 이웃 높이로 몰아 주므로(docToFlow.groupLanesByCable) "레인이
   * 갈려서" 못 그리는 일은 없어졌다. 남는 것은 **끝단이 서로 반대쪽인 배치** 뿐이다 —
   * 그때는 두 심선이 정말로 함께 가는 구간이 없다. 화면이 그 사실을 말하지 않으면
    ...
    <div className="pp-cable">
      ...
      */}
      <p className="pp-hint indent">
        ...
        {drawn === 0
          && ' 지금 배치에서는 자켓이 그려지지 않습니다 —'
          + ' 심선들이 끝단으로 곧장 갈라져 함께 가는 구간이 없습니다.'
          + ' 끝단을 도면에서 같은 쪽으로 모으면 그 구간이 생기고 자켓이 나갑니다.'}
      </p>
